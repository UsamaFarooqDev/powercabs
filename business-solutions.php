...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $requiredMissing = $old['beneficial_owner'] === '' || $old['taxi_driver'] === '' || $old['first_name'] === ''
        || $old['last_name'] === '' || $old['email'] === '' || $old['dob'] === '' || $old['nationality'] === ''
        || $old['iban'] === '' || $old['account_name'] === '' || $old['bank'] === ''
        || $old['address_same_as_statement'] === '' || $old['device_type'] === '';
    ...
    if ($requiredMissing) {
        ...
        if (!in_array($mime, $allowedMime, true) || $_FILES['bank_statement']['size'] > 5 * 1024 * 1024) {
            ...
            $body = "New PowerCabs Payment Solutions application.\n\n"
                  . "Beneficial Owner: " . ucfirst($old['beneficial_owner']) . "\n"
                  . "Taxi Driver: " . ucfirst($old['taxi_driver']) . "\n"
                  . "Name: {$fullName}\n"
                  . "Email: {$old['email']}\n"
                  . "Date of Birth: {$old['dob']}\n"
                  . "Nationality: {$old['nationality']}\n\n"
                  . "IBAN: " . strtoupper(str_replace(' ', '', $old['iban'])) . "\n"
                  . "Account Name: {$old['account_name']}\n"
                  . "Bank: {$old['bank']}\n"
                  . "Permanent address same as bank statement: " . ucfirst($old['address_same_as_statement']) . "\n\n"
