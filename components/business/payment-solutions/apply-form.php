      ...
      <form method="post" action="#payment-apply-form" class="row g-3" enctype="multipart/form-data">
        <div class="col-md-6">
          ...
          <div class="d-flex gap-2">
            <input type="radio" class="btn-check" id="paBeneficialYes" name="beneficial_owner" value="yes" autocomplete="off" required <?= $old[
              'beneficial_owner'
            ] === 'yes'
              ? 'checked'
              : '' ?>>
            <label class="btn btn-outline-primary rounded-pill fw-semibold px-4" for="paBeneficialYes">Yes</label>
            <input type="radio" class="btn-check" id="paBeneficialNo" name="beneficial_owner" value="no" autocomplete="off" <?= $old[
              'beneficial_owner'
            ] === 'no'
              ? 'checked'
              : '' ?>>
        ...
        <div class="col-md-6">
          ...
          <div class="d-flex gap-2">
            <input type="radio" class="btn-check" id="paTaxiYes" name="taxi_driver" value="yes" autocomplete="off" required <?= $old[
              'taxi_driver'
            ] === 'yes'
              ? 'checked'
              : '' ?>>
            <label class="btn btn-outline-primary rounded-pill fw-semibold px-4" for="paTaxiYes">Yes</label>
            <input type="radio" class="btn-check" id="paTaxiNo" name="taxi_driver" value="no" autocomplete="off" <?= $old[
              'taxi_driver'
            ] === 'no'
              ? 'checked'
              : '' ?>>
        ...
        <div class="col-md-4">
          ...
          <select class="form-select pc-custom-select-enhance" id="paTitle" name="title">
            ...
            <?php foreach (['Mr', 'Mrs', 'Ms', 'Miss', 'Dr', 'Other'] as $titleOption): ?>
              <option value="<?= htmlspecialchars($titleOption) ?>" <?= $old['title'] === $titleOption
  ? 'selected'
  : '' ?>><?= htmlspecialchars($titleOption) ?></option>
            <?php endforeach; ?>
          </select>
        </div>
        <div class="col-md-4">
          <label class="form-label pc-required" for="paFirstName">First Name</label>
          <input type="text" class="form-control" id="paFirstName" name="first_name" placeholder="Enter your first name" value="<?= htmlspecialchars(
            $old['first_name'],
          ) ?>" required>
        </div>
        <div class="col-md-4">
          <label class="form-label pc-required" for="paLastName">Last Name</label>
          <input type="text" class="form-control" id="paLastName" name="last_name" placeholder="Enter your last name" value="<?= htmlspecialchars(
            $old['last_name'],
          ) ?>" required>
        </div>
        ...
        <div class="col-md-6">
          ...
          <input type="email" class="form-control" id="paEmail" name="email" placeholder="Email address" value="<?= htmlspecialchars(
            $old['email'],
          ) ?>" required>
        </div>
        <div class="col-md-6">
          <label class="form-label pc-required" for="paDob">Date of Birth</label>
          <input type="date" class="form-control pc-custom-datetime-enhance" id="paDob" name="dob" value="<?= htmlspecialchars(
            $old['dob'],
          ) ?>" max="<?= date('Y-m-d') ?>" data-dt-quick-year="1" required>
        </div>
        ...
        <div class="col-md-6">
          ...
          <select class="form-select pc-custom-select-enhance" id="paNationality" name="nationality" required>
            ...
            <?php foreach ($countries as $country): ?>
              <option value="<?= htmlspecialchars($country) ?>" <?= $old['nationality'] === $country
  ? 'selected'
  : '' ?>><?= htmlspecialchars($country) ?></option>
            <?php endforeach; ?>
          </select>
        </div>
        <div class="col-md-6">
          <label class="form-label pc-required" for="paIban">IBAN</label>
          <input type="text" class="form-control text-uppercase" id="paIban" name="iban" placeholder="e.g. IE29AIBK93115212345678" value="<?= htmlspecialchars(
            $old['iban'],
          ) ?>" required>
        </div>
        ...
        <div class="col-md-6">
          ...
          <input type="text" class="form-control" id="paAccountName" name="account_name" value="<?= htmlspecialchars(
            $old['account_name'],
          ) ?>" required>
        </div>
        <div class="col-md-6">
          <label class="form-label pc-required" for="paBank">Bank</label>
          <input type="text" class="form-control" id="paBank" name="bank" value="<?= htmlspecialchars(
            $old['bank'],
          ) ?>" required>
        </div>
        ...
        <div class="col-md-6">
          ...
          <div class="d-flex gap-2">
            <input type="radio" class="btn-check" id="paAddressYes" name="address_same_as_statement" value="yes" autocomplete="off" required <?= $old[
              'address_same_as_statement'
            ] === 'yes'
              ? 'checked'
              : '' ?>>
            <label class="btn btn-outline-primary rounded-pill fw-semibold px-4" for="paAddressYes">Yes</label>
            <input type="radio" class="btn-check" id="paAddressNo" name="address_same_as_statement" value="no" autocomplete="off" <?= $old[
              'address_same_as_statement'
            ] === 'no'
              ? 'checked'
              : '' ?>>
        ...
        <div class="col-12">
          ...
          <select class="form-select pc-custom-select-enhance" id="paDeviceType" name="device_type" required>
            ...
            <?php foreach (['PAX A50', 'PAX A920', 'EPOS'] as $device): ?>
              <option value="<?= htmlspecialchars($device) ?>" <?= $old['device_type'] === $device
  ? 'selected'
  : '' ?>><?= htmlspecialchars($device) ?></option>
            <?php endforeach; ?>
          </select>
        </div>
